...
`grounded-formation-v4` writer and `reliable-v1` associative presentation reuse
EVENT, ENTITY, the Memory backend, IngestionStateStore, Cold and FirstHit.
...
memory = MagmaMemoryAdapter.create_real(
    ...
    ingestion_version="grounded-formation-v4", formation_model=model,
...
repaired identity issue. Any other syntax defect — an unknown identity
...
results cannot displace the protected direct subset. This does not promise to
preserve every old result or make every seed relevant.

Complete Facts are the base representation. An optional single bounded Cold
read can replace a Fact view with its complete supporting source package.
Shared exact source ranges appear once; each actual turn/range counts as an
item. Unread gaps are never filled. An oversized/incomplete source upgrade
falls back to the complete selected Fact. An oversized Fact is omitted whole.
...
`fact`/`sources` representation. `facts.evidence` retains selected Fact DTOs for
traceability even when their text is replaced by sources. Only `rendered_text`
...
- `current_user` identity has no deterministic source-role backstop in v4:
  ...
  check, not on a code rule. A deterministic guard belongs in a future
  pre-freeze revision; changing it now would alter frozen stage parses.
- The G1 prompt asks for identity evidence only on `same_as` and `named`
  ...
  to the recorded G2 vote (logged as a repaired identity issue). Aligning the
  prompt and parser belongs to a future formation version because it would
  alter frozen stage request digests and saved parses. Re-ingesting a
  completed v4 window whose deferred bindings change fails closed
...
- When a caller-requested source package replaces a Fact view, the model sees
  a short handle plus the source ranges, not the full canonical text; the Fact
  DTO remains only for traceability.
- A non-verdict G-stage failure (delivery unknown, unparseable output, or a
  decision manifest that does not exactly match the proposal) strands the
  window's accepted body text: v4 has no repair or resampling path, so the
  window stays pending and Cold is never consumed. One `insufficient` F2
  verdict or one invalid F1 entry likewise leaves the segment permanently
  partial. These are deliberate terminal states, not silent success.
