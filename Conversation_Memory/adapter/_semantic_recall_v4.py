"""Request-local locked base and optional graph-only semantic supplement."""
...
from .semantic_protocol import usage_guidance
...
INTENTS = frozenset({"none", "same_event_detail", "analogy", "disambiguation", "boundary"})
...
def prepare_base(adapter, query, policy):
    ...
    diagnostics["profile"] = "semantic-associative-v4"
...
def lock_base(adapter, base: PreparedRecall, suggestions, seek_graph: bool,
              graph_intent: str) -> LockedBaseSelection:
    if type(seek_graph) is not bool or graph_intent not in INTENTS or (
        seek_graph == (graph_intent == "none")
    ):
    ...
    limited = replace(base, _semantic_final_limits=(2 if seek_graph else 3,
                                                    FINAL_CHARS, FINAL_BYTES))
    ...
    return LockedBaseSelection(
        ...
        seek_graph=seek_graph, graph_intent=graph_intent,
...
def prepare_graph_supplement(adapter, lock: LockedBaseSelection, policy):
    ...
    diagnostics = {"profile": "semantic-associative-v4", "graph_exclusive_pool_ids": (),
    ...
    try:
        ...
        diagnostics["candidate_scores"] = {eid: {key: row[key] for key in
            ("group", "h", "cosine", "lexical", "node_id")}
            for eid, row in pool.items()}
        groups = sorted({row["group"] for row in pool.values()})
        def first_key(row):
            return (-row["h"], -row["cosine"], -row["lexical"], row["item"].evidence_id)
        def fill_key(row):
            return (-row["h"], -max(row["cosine"], row["lexical"]), row["item"].evidence_id)
