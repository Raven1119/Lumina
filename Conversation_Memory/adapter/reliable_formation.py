...
def form_reliable_batch(segment, model, *, progress=None, checkpoint, identity_candidates=()):
    ...
    progress = deepcopy(progress) if progress is not None else {"schema_version": PROGRESS_VERSION, "stages": {}}
    if (not isinstance(progress, dict) or set(progress) - {"schema_version", "stages", "identity_candidates"}
            or progress.get("schema_version") != PROGRESS_VERSION or not isinstance(progress.get("stages"), dict)
