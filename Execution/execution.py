...
class EventLog:
    ...
    def _validate_append(
        ...
        if event_type == "INTERRUPT_REQUESTED":
            ...
            if (
                cause_event is not None
                and cause_event.event_type == "IPYTHON_EXECUTION_STARTED"
            ):
                child_ref = payload["child_ref"]
                state = fold_execution_state(self.events)
                child_limit = (
                    1 if state.max_depth == 2 else _MAX_CHILDREN_PER_ROOT
                )
                if (
                    previous.event_id != cause_event.event_id
                    or state.status != "running"
                    or not isinstance(child_ref, ChildRef)
                    or not child_ref.local_goal
                    or len(child_ref.local_goal) > 1_024
                    or child_ref.provider_tool_call_id
                    != cause_event.payload.get("provider_tool_call_id")
                    or child_ref.parent_actor_id != state.actor_id
                    or state.depth >= state.max_depth
                    or child_ref.depth != state.depth + 1
                    or child_ref.max_depth != state.max_depth
                    ...
                    or len(state.child_refs) >= child_limit
                ):
                    raise ValueError(
                        "IPython Child spawn must match its Host-owned admission"
                    )
                return
            decision_event = next(
                (
            ...
            if previous.event_type == "CHILD_SPAWNED":
                ipython_start = next(
                    (
                        event
                        for event in self._events
            ...
            elif previous.event_type == "INTERRUPT_REQUESTED":
                ipython_start = next(
                    (
                        event
                        for event in self._events
...
def _decision_advisory_for(
    ...
    if type(advisory) is not tuple or len(advisory) != 2:
        return None
    target_decision_id, context = advisory
    if (
        type(target_decision_id) is not str
        or target_decision_id != decision_id
        or type(context) is not str
        or not context.strip()
        or context != context.strip()
        or len(context) > _MAX_DECISION_ADVISORY_CHARS
    ):
        return None
    return context


def _context_with_decision_advisory(
    context: str,
    advisory: str | None,
    max_chars: int,
) -> str:
    ...
    if _DECISION_ADVISORY_CONTEXT_KEY in document:
        raise ValueError("execution context already contains an advisory")
    document[_DECISION_ADVISORY_CONTEXT_KEY] = advisory
...
def _context_without_decision_advisory(context: str) -> str:
    marker = f'"{_DECISION_ADVISORY_CONTEXT_KEY}"'
    if marker not in context:
        return context
    document = json.loads(context)
    if _DECISION_ADVISORY_CONTEXT_KEY not in document:
        return context
    document.pop(_DECISION_ADVISORY_CONTEXT_KEY)
    return json.dumps(
        document,
        ensure_ascii=False,
        separators=(",", ":"),
...
def _build_model_request(
    ...
    context = _context_with_decision_advisory(
        context,
        decision_advisory_context,
        max_context_chars,
    )
    return ModelRequest(
        context=context,
...
class RootAgentProcess:
    ...
    def __init__(
        ...
        *,
        ...
        model_tool_names = {
            contract.partition("(")[0] for contract in model_contracts
        }
        model_facing_spawn = self._child_limit and "ipython" not in model_tool_names
        allowed_names = (
            {"read", "write", "shell", "ipython", "wait", "return"}
            | ({"spawn_child"} if model_facing_spawn else set())
            if self._actor_role == "child"
            else (
        ...
        if self._event_log.events:
            ...
            if last_event.event_type == "CHILD_SPAWNED":
                spawn_cause = next(
                    (
                        event
                        for event in self._event_log.events
                        if last_event.source_event_refs == (event.event_id,)
                    ),
                    None,
                )
                if (
                    spawn_cause is not None
                    and spawn_cause.event_type == "IPYTHON_EXECUTION_STARTED"
                ):
                    raise ValueError(
        ...
        if self._ipython_child_delegation and isinstance(
            self._ipython_control,
            PersistentIPython,
        ):
            self._ipython_control.bind_spawn_child_handler(
                self._spawn_child_from_ipython
            )
    ...
    def _append_action_outcome(
        ...
        source_refs = (
            (cause.event_id,)
            if cause.event_type in ("INTERRUPT_REQUESTED", "CHILD_SPAWNED")
    ...
    def _spawn_child_from_ipython(self, goal: str) -> Mapping[str, object]:
        with self._lifecycle:
            state = self._current_state()
            cause = self._active_ipython_event
            if not isinstance(goal, str) or not goal or len(goal) > 1_024:
                raise ValueError("goal must contain 1 to 1024 characters")
            if not self._ipython_child_delegation or cause is None:
                raise ValueError("spawn_child is unavailable for this Actor")
            if self._active_ipython_batch_size != 1:
                raise ValueError(
                    "spawn_child requires one IPython call in the decision"
                )
            if not self._child_limit or len(state.child_refs) >= self._child_limit:
                raise ValueError("child_limit_reached")
            if state.depth >= state.max_depth:
                raise ValueError("max_depth_reached")
            if self._event_log.path is None:
                raise ValueError("child_persistence_required")
            if self._event_log.events[-1].event_id != cause.event_id:
                raise ValueError("spawn_child admission is no longer current")
            child_ref = self._append_child_spawn(
                goal,
                cause.payload.get("provider_tool_call_id"),
                cause,
            )
            return {
                "child_execution_id": child_ref.child_execution_id,
                "child_actor_id": child_ref.child_actor_id,
                "parent_actor_id": child_ref.parent_actor_id,
                "depth": child_ref.depth,
                "max_depth": child_ref.max_depth,
            }
        ...
        for action, provider_call_id in zip(
            ...
            if isinstance(action, IPythonCode):
                with self._lifecycle:
                    ...
                    self._active_ipython_event = execution_event
                    self._active_ipython_batch_size = len(actions)
        ...
        *,
        ...
        while True:
            ...
            request = _build_model_request(
                ...
                (
                    (
                        "IPython provides spawn_child(goal) for delegating "
