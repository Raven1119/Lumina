...
| semantic-associative-v3，显式后继候选 | `LUMINA_MEMORY_PROFILE=semantic-associative-v3`，缺省 `LUMINA_MIND_GATE_MODE=llm`；图无关 BasePanel 是 G 的字节相同前缀，图独有 Fact 只能追加；Mind 先判事件身份，Answer 不把局部缺失说成全局无记录；默认不变 | `Conversation_Memory/.venv/bin/python -m pytest Conversation_Memory/tests/test_calibrated_first_hit.py tests/test_semantic_associative_v3_chat.py -q`；[v3 契约](../Conversation_Memory/docs/RELIABLE_MEMORY.md#semantic-associative-v3-explicit-reader) |
| semantic-associative-v4，显式后继候选 | `LUMINA_MEMORY_PROFILE=semantic-associative-v4`，`LUMINA_MIND_GATE_MODE=llm`；先以图无关 BasePanel 锁定最多 2/3 条 Fact，再按 `seek_graph` 惰性读取图，第二次 Mind 只能追加剩余 slot。图失败保留 base，Answer 缺失断言只审计不改写；默认不变 | `Conversation_Memory/.venv/bin/python -m pytest Conversation_Memory/tests/test_semantic_recall_v4.py tests/test_semantic_associative_v4_chat.py -q`；[v4 契约](../Conversation_Memory/docs/RELIABLE_MEMORY.md#semantic-associative-v4-explicit-reader) |
| semantic-associative-v5，显式后继候选 | `LUMINA_MEMORY_PROFILE=semantic-associative-v5`，`LUMINA_MIND_GATE_MODE=llm`；Base 始终先锁定最多 3 条，图只在请求且有空位时运行。`graph_need` 仅排序 FirstHit 已访问的图独有池，schema 枚举同源，Answer grounding 风险只审计；默认不变 | `Conversation_Memory/.venv/bin/python -m pytest Conversation_Memory/tests/test_semantic_recall_v5.py tests/test_semantic_associative_v5_chat.py -q`；[v5 契约](../Conversation_Memory/docs/RELIABLE_MEMORY.md#semantic-associative-v5-explicit-reader) |
| semantic-associative-v6，显式后继候选 | `LUMINA_MEMORY_PROFILE=semantic-associative-v6`，`LUMINA_MIND_GATE_MODE=llm`；锁定最多 3 条 Base 后，每次健康 Memory 读取都本地探索 FirstHit，图独有补充最多 1 条；最终最多 4 条，仍限 5000 字符/20000 字节。D6 直接索引补充只在隔离实验中运行，未成为生产 profile；默认不变 | `Conversation_Memory/.venv/bin/python -m pytest Conversation_Memory/tests/test_semantic_recall_v6.py tests/test_semantic_associative_v6_chat.py -q`；[v6 契约](../Conversation_Memory/docs/RELIABLE_MEMORY.md#semantic-associative-v6-explicit-reader) |
| semantic-associative-v7，显式记忆使用候选 | `LUMINA_MEMORY_PROFILE=semantic-associative-v7`，`LUMINA_MIND_GATE_MODE=llm`；逐字复用 v6 读取和容量，只改变 Mind 的具体价值判断及 Answer 的单一 CURRENT/HISTORY/RESPONSE 协议；默认不变。冻结 Transfer-5 的 12 题候选选择完成，10 题四臂 Answer 完成，另两题受 128 次调用硬上限约束未生成；观察到类比错标 history 和无证据的过去叙述，**不晋升** | `Conversation_Memory/.venv/bin/python -m pytest Conversation_Memory/tests/test_semantic_recall_v7.py tests/test_semantic_associative_v7_chat.py -q`；[v7 契约与未完成评估](../Conversation_Memory/docs/RELIABLE_MEMORY.md#semantic-associative-v7-explicit-reader) |
...
| 图单调追加＋事件边界 / `semantic-associative-v3`：显式 `LUMINA_MEMORY_PROFILE=semantic-associative-v3` | 复用 v2 的种子/索引基础选择，但 BasePanel 不依赖图传播；G 在字节相同的 BasePanel 后追加最多 8 条图独有 Fact，不会挤掉 B 中可见记忆。Mind 一次判历史/类比；Answer 有局部检索缺失约束。冻结 22 题前缀均成立，旧类比误标降至 2/6；但 G 仍有选材分心与局部缺失的无记录断言，图独有收益未在断边恢复三步中稳定重现，**不晋升** | [候选契约与实际限制](../Conversation_Memory/docs/RELIABLE_MEMORY.md#semantic-associative-v3-explicit-reader)；`Conversation_Memory/.venv/bin/python -m pytest Conversation_Memory/tests/test_calibrated_first_hit.py tests/test_semantic_associative_v3_chat.py -q`；冻结题、图、gold、回执和回答只保留本地 |
| 锁定基础选择＋按需图补充 / `semantic-associative-v4`：显式 `LUMINA_MEMORY_PROFILE=semantic-associative-v4` | v3 的图无关 BasePanel 由第一次 Mind 独立选择并锁定；仅在 `seek_graph=true` 且留有 final slot 时运行原 FirstHit、提供最多 24 条 graph-only 卡片。第二次 Mind 的合法选择只能追加，故障回到锁定 base；最终最多 3 Fact。Answer 使用非穷尽覆盖标记，确定性全局缺失检测仅审计。冻结 18 题中仅 2 题请求图，两次图选择均为空；B4/G4 的 12 对 Answer 输入相同，未观察到图收益。首批 known 协议有 4 次解析拒绝、1 次 UNKNOWN，保留而未重试；Answer 仍有全局缺失断言，**不晋升** | [候选契约与限制](../Conversation_Memory/docs/RELIABLE_MEMORY.md#semantic-associative-v4-explicit-reader)；`Conversation_Memory/.venv/bin/python -m pytest Conversation_Memory/tests/test_semantic_recall_v4.py tests/test_semantic_associative_v4_chat.py -q`；正式题、图、gold、回执与回答仅留本地 |
| 完整 Base 保留＋缺口引导图补充 / `semantic-associative-v5`：显式 `LUMINA_MEMORY_PROFILE=semantic-associative-v5` | v4 同路线后继，保留 v4 复现入口。Base 无论请求图与否都可锁定 3 Fact；仅有真实空位才访问图。第一次 Mind 的 `graph_need` 仅对 FirstHit 已访问的图独有池做本地排序，第二次 Mind 只能追加。合成协议解析 6/6，但预期触发图的合成题也返回 `seek_graph=false`；冻结 Transfer-3 的 12 题中 9 题完整、2 题部分、1 题漏掉具体图线索。18 题正式选择均未请求图，18 对 B5/G5 Answer 请求相同且精确去重，图收益未获验证；既有 U09 仍将“今日将发”升级为“已发”，**不晋升** | [候选契约与限制](../Conversation_Memory/docs/RELIABLE_MEMORY.md#semantic-associative-v5-explicit-reader)；`Conversation_Memory/.venv/bin/python -m pytest Conversation_Memory/tests/test_semantic_recall_v5.py tests/test_semantic_associative_v5_chat.py -q`；冻结题、gold、图、回执与实际回答只留本地 |
| 始终探索＋单条后补与容量归因 / `semantic-associative-v6`：显式 `LUMINA_MEMORY_PROFILE=semantic-associative-v6` | v5 同路线后继，保留旧入口。Base 完整锁定最多 3 条，健康读取本地探索一次 FirstHit；direct/index 与 graph-exclusive 池使用同一选择协议及 24 卡预算，最多追加 1 条，最终 4 Fact ceiling 但 5000 字符/20000 字节不增。冻结 Transfer-4 的 12 题中 D6 未追加，G6 一题追加图独有 Fact 并提供具体器具边界；该 Answer 同时有不受支持细节，且 B6 仍否认可见证据，故**不晋升**。6 次真实 wrapper 诊断中计划状态 3/3 正确，另一组存在身份歧义，不作为修复证据 | [候选契约与限制](../Conversation_Memory/docs/RELIABLE_MEMORY.md#semantic-associative-v6-explicit-reader)；`Conversation_Memory/.venv/bin/python -m pytest Conversation_Memory/tests/test_semantic_recall_v6.py tests/test_semantic_associative_v6_chat.py -q`；正式材料仅在本地 |
| 具体价值选择＋统一 Answer 协议 / `semantic-associative-v7`：显式 `LUMINA_MEMORY_PROFILE=semantic-associative-v7` | v6 的同读取后继，保留 v6 复现入口；候选 ID/卡片/顺序/支持字段在冻结 18 题及 Transfer-5 的 12 题上相等（耗时遥测除外）。v7 可保留当前消息已足够时真正有用的过去，但冻结选择中多次把不同事件标成 same_event。固定 Memory 的 8 题 Answer hard-error case 从 4 降到 2；Transfer-5 的 10 题完整四臂里，v7 Answer 又在无 Memory 时编造蓝墨图经过，各臂还把“决定送审”说成“已送审”。另外两题未生成 Answer，不能宣称完整 factorial 或晋升，**目前判为回归** | [候选契约及评估边界](../Conversation_Memory/docs/RELIABLE_MEMORY.md#semantic-associative-v7-explicit-reader)；`Conversation_Memory/.venv/bin/python -m pytest Conversation_Memory/tests/test_semantic_recall_v7.py tests/test_semantic_associative_v7_chat.py -q`；正式题、图、gold、回执和回答仅留本地 |
